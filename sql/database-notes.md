...
```
BEGIN;
UPDATE balances SET amount -= 100 WHERE id = 1;
UPDATE balances SET amount += 100 WHERE id = 2;
ROLLBACK;
COMMIT;
```
...
- READ COMMITTED - решает аномалии на грязное чтение, но не решает остальные.
- REPEATABLE READ - решает все виды аномалий, но только в PostgreSQL, в самом SQL не защищает от фантомного чтения.
- SERIALIZABLE - решает все виды аномалий
