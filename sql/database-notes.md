...
- `timestamptz` против `timestamp` - Оба метода применяются для времени, timestamptz может преобразовать время в часовой пояс установленном в `TimeZone` сессии. 
- `uuid` - Уникальные идентификаторы.
- `jsonb` - Бинаргный формат json.

## Что такое нормализация? Виды нормализации.

- нормализация - это процесс приведения структуры БД к виду, устраняющему избыточность данных:
  - устранения с дублированием данных, 
  - устранение аномалии вставки/обновления/удаления.

1. 1НФ (Первая нормальная форма) - в каждой ячейке атомарное значение.
2. 2НФ (Вторая нормальная форма) - все поля полностью зависят от всего составного первичного ключа.
...
- RIGHT JOIN -  это такое же объединение, как и **LEFT JOIN**, но выводятся все записи из правой таблицы, а к ним добавляются только те данные из левой таблицы, в которых выполняется условие, если данные в левой части отсутсвуют то там будет стоять `NULL`.
 **Пример**
...
7. Оконные функции — вычисляются `OVER()` (после `HAVING`, до `ORDER BY`).
8. ORDER BY — сортируем по условию.
9. LIMIT — отрезаем первые N строк.
...
**Почему в `ORDER BY` можно использовать алиас из `SELECT`**
- `ORDER BY` может выполнять данные из `SELECT`, потому что сортировка выполняется после `SELECT` и он видит их
...
FROM Group_class
WHERE mark > (SELECT AVG(mark) FROM Group_class)
```

2. Коррелированные - это подзапрос в `SELECT`, `FROM`, `WHERE`, который ссылается на внешнюю таблицу из главного запроса, выполняется для каждой строки внешнего запроса, что делает их дорогими и сложность данного алгоритма будет О(n * m) для 1000 элементов не кретично, но для 100 млн записей база данных будет иметь очень большую сложность алгоритма.
...
## CTE?
- **CTE** - это обощенная табличное выражение, которая является особой формой написания именнованного подзапроса. В PostgreSQL ≤ 11 CTE всегда материализовался — вычислялся один раз и хранился в памяти, поэтому к нему легко добраться.В PostgreSQL 12+ неявно материализованных CTE больше нет: одиночно используемый нерекурсивный CTE инлайнится в основной запрос, как обычный подзапрос.
- Неявная материализация PostgreSQL сам решает:
  - CTE используется один раз → инлайнится (склеивается с основным запросом)
  - CTE используется два и более раз → материализуется (вычисляется отдельно)
- Явная материализация: `WITH ... AS MATERIALIZED (...)`. Принудительно заставляет PostgreSQL вычислить CTE отдельно и сохранить результат.
CTE НЕ склеивается с основным запросом — это optimization fence. Материализованный CTE может уйти на диск, а не «храниться в оперативной памяти».
- Явная материализация: `WITH ... AS NOT MATERIALIZED (...)`. Принудительно заставляет PostgreSQL инлайнить CTE в основной запрос, даже если он используется несколько раз.
...
```
- 1-запрос без `MATERIALIZED` инлайнится к основному запросу:
```
EXPLAIN (ANALYZE, BUFFERS)
WITH avg_mark AS (
    SELECT group_id, AVG(mark) AS avg_val
    FROM students
    GROUP BY group_id
)
SELECT s.name
FROM students s
JOIN avg_mark a ON s.group_id = a.group_id
WHERE s.mark > a.avg_val;
```
- Результат:
```
Hash Join  (cost=223.15..431.37 rows=3333 width=12) (actual time=3.045..7.510 rows=5022 loops=1)
   Hash Cond: (s.group_id = students.group_id)
   Join Filter: ((s.mark)::numeric > (avg(students.mark)))
   Rows Removed by Join Filter: 4978
   Buffers: shared hit=146
   ->  Seq Scan on students s  (cost=0.00..173.00 rows=10000 width=20) (actual time=0.009..0.903 rows=10000 loops=1)
         Buffers: shared hit=73
   ->  Hash  (cost=223.07..223.07 rows=6 width=36) (actual time=3.023..3.024 rows=6 loops=1)
         Buckets: 1024  Batches: 1  Memory Usage: 9kB
         Buffers: shared hit=73
         ->  HashAggregate  (cost=223.00..223.07 rows=6 width=36) (actual time=3.016..3.019 rows=6 loops=1)
               Group Key: students.group_id
               Batches: 1  Memory Usage: 24kB
               Buffers: shared hit=73
               ->  Seq Scan on students  (cost=0.00..173.00 rows=10000 width=8) (actual time=0.003..1.027 rows=10000 loops=1)
                     Buffers: shared hit=73
 Planning:
...
```
- 2-запрос c `MATERIALIZED` инлайнится к основному запросу:
```
EXPLAIN (ANALYZE, BUFFERS)
WITH avg_mark AS MATERIALIZED (
    SELECT group_id, AVG(mark) AS avg_val
    FROM students
    GROUP BY group_id
)
...
- Результат 
```
Hash Join  (cost=223.27..583.77 rows=3333 width=12) (actual time=3.066..7.730 rows=5022 loops=1)
   Hash Cond: (s.group_id = a.group_id)
   Join Filter: ((s.mark)::numeric > a.avg_val)
   Rows Removed by Join Filter: 4978
   Buffers: shared hit=146
   CTE avg_mark
     ->  HashAggregate  (cost=223.00..223.07 rows=6 width=36) (actual time=2.972..2.976 rows=6 loops=1)
           Group Key: students.group_id
           Batches: 1  Memory Usage: 24kB
           Buffers: shared hit=73
           ->  Seq Scan on students  (cost=0.00..173.00 rows=10000 width=8) (actual time=0.002..0.767 rows=10000 loops=1)
                 Buffers: shared hit=73
   ->  Seq Scan on students s  (cost=0.00..173.00 rows=10000 width=20) (actual time=0.006..0.806 rows=10000 loops=1)
         Buffers: shared hit=73
   ->  Hash  (cost=0.12..0.12 rows=6 width=36) (actual time=2.991..2.992 rows=6 loops=1)
         Buckets: 1024  Batches: 1  Memory Usage: 9kB
         Buffers: shared hit=73
         ->  CTE Scan on avg_mark a  (cost=0.00..0.12 rows=6 width=36) (actual time=2.975..2.980 rows=6 loops=1)
               Buffers: shared hit=73
 Planning:
...
```
**Вывод**
- В первом эксперименте мы видим, что планировщик склеивает подзапрос с основным запрос, а во втором эксперименте, CTE подзапрос материализуется и вычисляется отдельно от основного запроса и он существует только во время выполнения одного запроса. После завершения — исчезает.
...
|   name  |avg_mark|
|---------|--------|
|  Тахир  |  4.6666666666666667  |
| Максим  |  4.6666666666666667   |

**Таблица `PARTITION BY`:**
|   name  |    object   |mark|
|---------|-------------|----|
|  Тахир  | Матеиматика |  5 |
|  Тахир  |    Физика   |  4 |
...
|---------|-------------|----|--------|
|  Тахир  | Матеиматика |  5 |  4.6666666666666667   |
|  Тахир  |    Физика   |  4 |  4.6666666666666667   |
|  Тахир  |    Химия    |  5 |  4.6666666666666667   |
| Максим  | Матеиматика |  5 |  4.6666666666666667   |
| Максим  |    Физика   |  4 |  4.6666666666666667   |
| Максим  |    Химия    |  5 |  4.6666666666666667   |
 ...
 **Пример**
 ...
 COMMIT;
 ```
 - `FOR SHARE` - блокирует строку для изменений, но позволяет другим читать и поддерживает одновременные транзакции.
...
### Модификаторы для дедлоков

- NOWAIT — модификатор для FOR UPDATE / FOR SHARE, который не ждёт освобождения блокировки, а сразу выбрасывает ошибку.
  ...
  **Синтаксис**
  ...
  ```
  **Вывод**
  - Вторая транзакция не будт ожидать, а просто упадет с ошибкой `ERROR:  could not obtain lock on row in relation "accounts"`

- Advisory-локи — это блокировки по произвольному числу, не привязанные к таблицам.
 ```
 -- Взять блокировку (блокирующая)
